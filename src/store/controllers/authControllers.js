        ...
        axios({
            ...
            if (userLoginRes.data.email) {
              ...
              console.log(userLoginRes.data);
                ...
                if(userLoginRes.data.message){
                  ...
                  reject({error: {message: userLoginRes.data.customMsg}, success:false});
                }
          ...
          .catch((err) => {
            ...
            switch(err.response.status){
              case 400: 
              reject({error: {message: err.response.data.customMsg}, success: false});
                break;
              case 502:
                if(attempt < 2){
                  this.loginUser(email, password, usertype, attempt +1).then(res => resolve(res)).catch(err => reject(err))
                }else{
                  reject({error: {message: err.response.data.message}, success: false});
                }
                break;
              default:
          ...
          axios({
            ...
            .then((playerStatsRes) => {
              if (playerStatsRes.data) {
                ...
                console.log(playerStatsRes.data);
                reject({error: {message: "Server is currently offline"}, success:false});
              }
            })
            .catch((err) => {
              console.log(
